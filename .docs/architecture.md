...
Event Settings lists every event type, including hidden ones (marked "Hidden"), refreshes on focus, and persists drag ordering. Tapping a row pushes the `edit-event` route with the type id. The Edit Event screen drafts label, icon, color pair, and the predictable/hidden flags behind a Save button, while option changes persist immediately through the data layer. It reuses the drag list (`react-native-sortables`) for option ordering, a bottom-sheet editor for adding/editing options, curated icon/color choices defined in the route file (event colors are data, not theme tokens), and status toasts for saves and deletes. Deleting an option or a custom event requires an `Alert` confirmation that states how many logged entries will also be removed, and the delete removes those history rows in the same transaction; system events cannot be deleted.
...
- Event Settings supports editing, hiding, reordering, and deleting custom events; creating new event types is not implemented.
